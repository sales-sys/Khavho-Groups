...
// Direct product loading function (bypasses real-time listeners)
window.loadProductsDirectly = async function() {
    ...
    try {
        ...
        if (!snapshot.empty) {
            ...
            snapshot.forEach((doc) => {
                ...
                console.log('📦 Direct product loaded:', productData.name || 'Unnamed', productData.category || 'No category');
                productsData.push(productData);
            });
            
            console.log('✅ Total products loaded directly:', productsData.length);
    ...
    try {
        ...
        window.db.collection('products').get()
            .then((testSnapshot) => {
                ...
                // Load products immediately using direct query (bypasses auth issues)
                console.log('🚀 Loading products immediately via direct query...');
                productsData = [];
                testSnapshot.forEach((doc) => {
                    const productData = { id: doc.id, ...doc.data() };
                    console.log('📦 Product loaded immediately:', productData.name || 'Unnamed', productData.category || 'No category');
                    productsData.push(productData);
                });
                
                console.log('✅ Total products loaded immediately:', productsData.length);
...
function handleProductsSnapshot(snapshot) {
    productsData = [];
    snapshot.forEach((doc) => {
        // Handle both real Firebase docs and mock data
        const productData = typeof doc.data === 'function' 
            ? { id: doc.id(), ...doc.data() }
            : { id: doc.id, ...doc };
